...
uint32_t CANRaw::init(uint32_t ul_baudrate)
{
	...
	// !!! Be sure DAC0 is not in use (audio for example) for CAN1, otherwise use only can0
	if (m_pCan == CAN0) {
		...
		pinMode(CAN0RX, INPUT); pinMode(CAN0TX, OUTPUT);  
		}
    if (m_pCan == CAN1) {
		...
		pinMode(CAN1RX, INPUT); pinMode(CAN1TX, OUTPUT);  
		}
...
//	if (enablePin != 255) { pinMode(enablePin, OUTPUT); digitalWrite(enablePin, HIGH); }
...
void CANRaw::global_send_transfer_cmd(uint8_t uc_mask)
{
	...
	//debug_nbsent++; // DEBUG SD - change to 1 and saturation of mailbox, meaning that nothing leaves
...
/**
 * \brief Send single mailbox transfer request. USED TO READ ONLY NOT TO SEND (SD)
 *
 * \param uc_index Indicate which mailbox is to be configured.
 */
