...
)
{
	...
	if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
		...
		if (res == FR_OK && (mode & FA_CREATE_ALWAYS)) {	/* Truncate it if overwrite mode */
			dw = get_fattime(Hour, Minute, Second, Month, Day, Year);					/* Created time */
	...
	if (res == FR_OK) {
		if (fp->flag & FA__WRITTEN) {	/* Has the file been written? */
			...
			if (res == FR_OK) {
				...
				tim = get_fattime(Hour, Minute, Second, Month, Day, Year);						/* Update updated time */
...
#else
	res = f_sync(fp, Hour, Minute, Second, Month, Day, Year);		/* Flush cached data */
#if _FS_SHARE
...
)
{
	...
	DIR dj;
	BYTE *dir, n;
	DWORD dsc, dcl, pcl, tim = get_fattime(Hour, Minute, Second, Month, Day, Year);
	DEF_NAMEBUF;
...
)
{
	...
	n = get_fattime(Hour, Minute, Second, Month, Day, Year);						/* Use current time as VSN */
	if (fmt == FS_FAT32) {
...
uint32_t get_fattime(int, int, int, int, int, int);
/**
 ...
 * \return Current time.
 */
uint32_t get_fattime(int Hour, int Minute, int Second, int Month, int Day, int Year)
{
	uint32_t ul_time;
