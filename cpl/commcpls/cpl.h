/*
	CPL.H - Driver Qualcomm QCA7000 CPL via SPI 
  ...
  MODIFICATION: ligne 87 A2 au lieu de A5 (port conv.analog-numérique)
*/
...
/* ATMEL SAM3X pins */
#define CPL_PWM     67  // output PWM PB16 - D67 - pin 77
#define CPL_FB_PWM  A8  // analogic input - A8 - pin 88
#define CPL_ENABLE  49  // output - default 0 - D49 - pin 96
...
int m=0;
    v[i]=analogRead(A8);  /* delayMicroseconds(100); */
    i++; if (i==10) i=0;
...
static char state=5;   // must be char, to check !!!?
  ...
  if (s>3840) { Serial.println(" - Disconnected (+12/-12)"); retour=0; return 1; } // 3600
  if (s>3100) { Serial.println(" - Connected to vehicle"); retour=1; return 1; } // 2900
  if (s>2816) { Serial.println(" - Disconnected");  retour=0; return 0; } // 2400
  if (s>2300) { Serial.println(" - Cable connected, no vehicle"); retour=0; return 0; } // 2100
 ...
 */
void cpl_pins(){
  pinMode(CPL_PWM, OUTPUT);
  pinMode(CPL_FB_PWM, INPUT);
  pinMode(CPL_INT, INPUT); 
  delay(CPL_RESET_TIMEOUT);
  pinMode(CPL_SELECT, OUTPUT); // not selected: high
  digitalWrite(CPL_SELECT, HIGH); 
  analogReadResolution(12); // 12 bits resolution - reading takes 100 microseconds - this line is not mandatory
}

/* PWM Generator */
void cpl_pwm_run() { 
   pwm_dac1.start(PWM_PERIOD_DAC1,PWM_DUTY_DAC1); 
   pinMode(CPL_ENABLE, OUTPUT);  
   digitalWrite(CPL_ENABLE, HIGH);
}

// dutycycle is like this: 50% = 50000
void cpl_pwm_change(uint32_t dutycycle) {
  if (dutycycle>PWM_PERIOD_DAC1) return;
  pwm_dac1.set_duty(dutycycle);
}

void cpl_pwm_stop() {
  pwm_dac1.stop();
  digitalWrite(CPL_ENABLE, LOW);
}

/* 
 * CPL start
 * station:with pwm,vehicle:no pwm
 *  CAUTION: don't change this order of code lines!!!!
 */
void cpl_start() {
  cpl_pins();
  delay(CPL_RESET_TIMEOUT);
