/*
  ISO 15118 board communication library
	...
	v1.0
	Sébastien Dourlens
  note: variant.h => PIN_SPI_SS3 redefined 48u for DIAG Board!
*/
...
// station MAC@
uint8_t recmac[]="\x00\x01\x87\x09\x27\x56";
char *recname="Station ERCTEEL";
// vehicle MAC@
uint8_t destmac[]="\x00\x01\x87\x09\x27\x6A";
char *destname="Vehicule";
...
void setup() {
  pinMode(LED1, OUTPUT);
	...
	cpl_start();
  cpl_check();
  Serial.println(F("Charger Board v1.0 (c) ERCTEEL"));
	Serial.println(F("Communication started... Enter any key to reset!"));
  delay(100);
...
void loop() {
  ...
  if (Serial.available()>0) { // COMMAND
       ...
       if (c=="h" || c=="H") {                
                ...
                strcpy(message,"Hello Vehicle, I am the station 1, Identify you please."); 
            ...
            Serial.print("Sending message: "+c+String(" - "));
