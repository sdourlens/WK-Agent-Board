...
int i,j;  
    ...
    for(i=1;i<9;i++) {
      Serial.print("\n\rTEST TOR "); Serial.print(i,DEC);  
      
      for(j=0;j<3;j++) {
        Serial.print("\nLOOP "); Serial.print(j,DEC);  Serial.println("/3"); 
      
        Serial.println("Command wire: +12V ...");  
        write_tor(i,HIGH);
        delay(2000);
        Serial.print("READ INPUT E: "); Serial.println(read_tor(i)); // check output 1 or 0
        Serial.print("READ DIAG (1=ok,2=failure/disconnected): "); Serial.println(read_diag(i)); //1=normal,0=failure
        Serial.print("READ ANA: ");Serial.println(read_ana(i)); // read voltage 0..4096
        delay(3000);
        
        Serial.println("Command wire: OV ...");  
        write_tor(1,LOW);
        delay(2000);
        Serial.print("READ INPUT E: "); Serial.println(read_tor(i)); // check output 1 or 0
        Serial.print("READ DIAG (1=ok,2=failure/disconnected): "); Serial.println(read_diag(i)); //1=normal,0=failure
      ...
      if (Serial.available()>0) { Serial.read(); Serial.flush(); break; }
      }
}
